...
public:
    ...
    */
    ...
    /*
    - What it does:
      Checks whether the board is unsolved and has no legal regular moves left.
    - Inputs:
      The current game state.
    - Outputs:
      True if the player is currently deadlocked.
    */
    static bool isDeadlocked(const GameState& state);

    /*
    - What it does:
      Applies the automatic deadlock penalty and grants a temporary empty tube.
    - Inputs:
      The current game state plus an output message string.
    - Outputs:
      True if a deadlock was resolved and the state was updated.
    */
    static bool resolveDeadlock(GameState& state, std::string& message);
};
