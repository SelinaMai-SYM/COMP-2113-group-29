...
/*
- What it does:
  Stores per-level special-mechanic state such as hidden colors, obscured tubes and deadlock relief.
- Inputs:
  None.
- Outputs:
  Metadata that augments the normal puzzle board without changing the underlying colors.
*/
struct MechanicState {
    std::vector<std::vector<bool>> hiddenBlocks;
    std::vector<bool> obscuredTubes;
    std::vector<char> obscuredUnlockTargets;
    int initialHiddenBlocks = 0;
    int initialObscuredTubes = 0;
    int deadlocksResolved = 0;
    int bonusEmptyTubesGranted = 0;
    int deadlockPenaltyMoves = 0;
...
/*
- What it does:
  Normalizes one stored color code so legacy and current saves behave the same way.
- Inputs:
  One raw block color character.
- Outputs:
  The canonical in-game color code.
*/
char canonicalColorCode(char color);

/*
- What it does:
  Converts one block color code into a player-facing color name.
- Inputs:
  One canonical or legacy color code.
- Outputs:
  A readable color label for prompts and UI text.
*/
